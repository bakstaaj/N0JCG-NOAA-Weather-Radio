...
OUT = ROOT / "docs" / "publications" / "N0JCG_NOAA_Weather_Radio_User_Guide_v1.0.0.pdf"
...
def footer(canvas, doc):
    ...
    canvas.drawString(.7 * inch, .35 * inch, "N0JCG NOAA Weather Radio | v1.0.0 | Receive-only by design")
    canvas.drawRightString(7.8 * inch, .35 * inch, f"Page {doc.page}")
    canvas.restoreState()

story = [P("N0JCG NOAA Weather Radio", "N0Title"), P("Operator User Guide | Product release v1.0.0"), P("A focused receive-only Raspberry Pi appliance for all seven US NOAA Weather Radio channels, FFT-directed strongest-channel selection, NFM audio, and operator-configurable SAME alert filters.")]
metadata_rows = [[P("<b>Metadata</b>"), P("<b>Value</b>")]] + [[P(key), P(value)] for key, value in (("Product", "N0JCG NOAA Weather Radio"), ("Product ID", "n0jcg-noaa-weather-radio"), ("Document", "Operator user guide"), ("Version", "1.0.0"), ("Status", "Product release"), ("Receiver", "RTL-SDR serial 00000162"), ("Audience", "Operators and installers"), ("Owner", "N0JCG"))]
