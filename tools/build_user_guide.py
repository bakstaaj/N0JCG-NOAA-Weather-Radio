...
DOCX = OUT / "N0JCG_NOAA_Weather_Radio_User_Guide_v0.1.0.docx"
...
footer = section.footer.paragraphs[0]; footer.alignment = WD_ALIGN_PARAGRAPH.CENTER; footer.add_run("N0JCG NOAA Weather Radio  |  v0.1.0  |  Receive-only by design")

title = doc.add_paragraph(style="Title"); title.add_run("N0JCG NOAA Weather Radio")
sub = doc.add_paragraph(); sub.add_run("Operator User Guide  |  Preview release v0.1.0").bold = True
...
h("First operation")
steps(["Confirm the receiver serial card shows 00000162.", "Press Scan strongest. The application surveys 162.395-162.555 MHz.", "Review candidate SNR values and the tuned channel.", "Press Stop before disconnecting the receiver or changing USB hardware."])
...
p("The scanner scores power around each canonical channel from one wide survey. The highest SNR candidate is selected, then rtl_fm is started in narrow-FM mode at 48 kHz. A high peak can still be interference; verify intelligible NOAA audio and inspect the other candidate values.")
h("SAME alerts")
...
h("Release boundary")
p("v0.1.0 includes software tests, deterministic simulation, UI, FFT scoring, NFM process control, SAME parsing, registration state, installer, package tooling, and this guide. Live USB, RF audio, antenna coverage, and end-to-end SAME acceptance remain target-hardware gates.")
doc.save(DOCX)
print(DOCX)
