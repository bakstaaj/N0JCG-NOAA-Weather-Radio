...
def score_channels(points: Iterable[FftPoint], half_width_hz: int = 8_000) -> list[ScanCandidate]:
    ...
    for channel in NOAA_CHANNELS:
        window = [p for p in points if abs(p.frequency_hz - channel.frequency_hz) <= half_width_hz]
        if window:
            peak_point = max(window, key=lambda point: point.power_dbfs)
            scored.append(ScanCandidate(channel, peak_point.frequency_hz, peak_point.power_dbfs, floor, peak_point.power_dbfs - floor))
